  ...
  for x in range(1, 10):
    try:
      ...
      for i in xrange(0, len(tr_list)-1, 2):
        ...
        if conf_title.lower().strip() in conferences.keys():
          # print "duplicate found"
          if cat not in conferences[conf_title.lower().strip()].categories:
            conferences[conf_title.lower().strip()].categories.append(cat)
          # print conferences[conf_title.lower().strip()].categories
        else:
          ...
          try:
            ...
            if conf_location.lower().strip() not in invalid_locations:
              ...
              if 'OK' == response.json()["status"]:
                ...
                conference = Conference(conf_title, conf_full_title, conf_url, conf_date, conf_location, conf_submission, conf_loc_info["lat"], conf_loc_info["lng"], cat)
              else:
                print response.json()
            ...
            if e.message:
              print e
            conference = Conference(conf_title, conf_full_title, conf_url, conf_date, conf_location, conf_submission, 0, 0, cat)
          conferences[conference.title.lower().strip()] = conference
      if finished: break
    except Exception, e:
      print e
